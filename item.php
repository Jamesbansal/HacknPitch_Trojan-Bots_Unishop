<?php include("header.php");
    ...
    if(isset($_GET['val']))
    {
      ...
      if($_SESSION["flag"]==1)
      {
      	$sql=mysqli_query($con," SELECT *  FROM product WHERE pname LIKE '%$val%' and university='$_SESSION[uni]'")or die('Error:- '.mysqli_error($con));
      }
      else{	$sql=mysqli_query($con," SELECT *  FROM product WHERE pname LIKE '%$val%' ")or die('Error:- '.mysqli_error($con));
